...
R/W Pin Read = LOW / Write = HIGH // if No pin connect RW , please leave
R/W Pin for Low State
Parameters
*/
#include "DHT.h"
 ...
 */
 int adc_zero1 = 500;
// relative digital zero of the arudino input from ACS712 (could make thisa variable and auto-adjust it)
 int adc_zero2 = 500;
// relative digital zero of the arudino input from ACS712 (could make thisa variable and auto-adjust it)
 int adc_zero3 = 500;
const int mi = 54 ;
// relative digital zero of the arudino input from ACS712 (could make this a variableand auto-adjust it)
...
void LCDCurrentStatus(long no1, long no2, long no3)
{
 ...
 // check if returns are valid, if they are NaN (not a number) then somethingwent wrong!
 lcd.setCursor(0,0);
 lcd.print(" ");
 lcd.setCursor(0,1);
 lcd.print(" ");
