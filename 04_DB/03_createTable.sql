...
create table Mod_Main(
	...
	mod_enabled varchar(1) not null default "1" comment '是否啟用:1:啟用、0:停用',
...
create table Mod_ParmData(
	...
	limit_enabled varchar(1) not null default "0" comment '是否啟用警示:1:啟用、0:停用',
	show_enabled varchar(1) not null default "1" comment '是否於修改畫面顯示:1:顯示、0:不顯示只能從DB修正',
    update_date timestamp not null DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP comment '更新時間',
	PRIMARY KEY(id),
...
create table Mod_Update_Log(
	...
	bfe_mod_enabled varchar(1) not null comment '修改前是否啟用',
	aft_mod_enabled varchar(1) not null comment '修改後是否啟用',
	update_date timestamp not null DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP comment '更新時間',
	PRIMARY KEY(id),
	FOREIGN KEY (mod_main_id) REFERENCES Mod_Main (id)
)comment '感應裝置更新紀錄';
...
create table Sen_Switch(
	id int not null AUTO_INCREMENT comment 'key值',
	mod_main_id int not null comment '感應裝置主檔id',
	pow_status varchar(1) not null comment '電源狀態:1:打開、0:關閉',
